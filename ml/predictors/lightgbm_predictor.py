...
Light Gradient Boosting Machine for crypto price direction prediction.
...
class LightGBMPredictor(BasePredictor):
    ...
    def __init__(self, symbol: str, config: Optional[Dict] = None):
        super().__init__(symbol, 'lightgbm')
        self.config = config or MODEL_CONFIG['lightgbm']
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
        ...
        logger.info(f"LightGBM trained for {self.symbol}: accuracy={self.metrics['accuracy']:.3f}")
        return self.metrics
